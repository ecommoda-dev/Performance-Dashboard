...
**الإصدار:** Worker `v3.5.0` · الواجهة `v3.5.0` ← الاتنين مستقلين، طبيعي يختلفوا
(اتصادفوا على نفس الرقم في التعديل ده بس — مش قاعدة)
...
آخر مطابقة: 30-08-2026 · `index.js` v3.5.0 · `index.html` v3.5.0
...
| `get_data` | GET/POST | البيانات المجمّعة — `boxes`/`rows`/`warnings` + `orderBoxes`/`orderRows`/`orderWarnings`. محتاج `dateFrom` و `dateTo`. الواجهة بتناديه **مرة لكل شهر بالتسلسل** (30-08-2026) وبتحسب كل الأرقام من `rows`/`orderRows` — الملخّصات (`boxes`/`orderBoxes`) بتوصف الـ chunk كامل فمش بتتعرض مباشرة |
...
- **🔴 التقسيم الشهري + مربعات الشهور + الفترة السريعة — 30-08-2026
  ...
  - 🆕 **مربعات الشهور** (Step 3-د-٣ · `monthly-chunk-loading.md` §9). صف
    ...
    **بيمسحوا بعض** (`resetRangePresetUI` ↔ `clearActiveMonthBox`).
    ...
    اتخذش هنا. ومفيش `STORE_START_DATE` في الأداة دي، فالمربعات مش مقصوصة على
    أي حد أدنى (بند مفتوح تحت).
...
- **مفيش `STORE_START_DATE` في الأداة دي.** مربعات الشهور بتولّد الشهر الحالي +
  ٥ سابقين **بلا حدّ أدنى**، والفترة السريعة لسه فيها "العام السابق". في
  `Logistics-Control-Center` الحد ده موجود (`2026-05-01` — بداية تشغيل المتجر،
  قرار أحمد 30-08-2026) وبيتطبّق على طبقتين (Worker + HTML). **مش متطبّق هنا
  عن قصد** — الأداة دي مالية وبتدعم فترات قديمة صراحةً، وإضافة حدّ ليها قرار
  تجاري لصاحب الأداة مش استنتاج. لو اتقرر، لازم يتحط في `index.js` **و**
  `index.html` في نفس التعديل.
